...
<body class="no-skin">
							...
							<div class="row">
								<div class="col-xs-12">
									...
									<div>
										...
										<c:if test="${not empty po.criteria.type }">
										...
										<div class="modal-content">
											<div class="modal-header no-padding">
												<div class="table-header">
													...
													<span id="modal-title">添加基础数据</span>
												</div>
											</div>

											<form action="mcCommonData_update.h" method="post" class="form-horizontal" role="form">
												<div class="modal-body">
													<div class="alert alert-danger hide"></div>
													<input type="hidden" name="id" id="data-id">
													<div class="form-group">
														<label for="name" class="col-sm-2 control-label">
															<font color="red">*</font>数据名称
														</label>
														<div class="col-sm-10">
													...
													<div class="form-group">
														...
														<div class="col-sm-10">
															<input type="text" class="form-control"
																name="sort" placeholder="请输入顺序" required>
														</div>
													</div>
													<div class="form-group">
	...
	<script type="text/javascript">
		...
		function editType(id) {
			$.ajax({
            	type: "POST",
                url: "ajax_get_mcCommonData.h",
                data: {"id" : id},
                dataType:'json',
                success: function(data){
                	$('#modal-title').html("编辑基础数据");
                	$('#modal-table').modal();
                	...
                	if(data && data.status){
                		...
                		if (0 == data.items.status) {
				    		$('.modal-body input[name="status"]').eq(0).removeAttr("checked");
				    		$('.modal-body input[name="status"]').eq(1).attr("checked","checked");
				    	}
                		$('.modal-body input[name="type"]').val(data.items.type);
                	}else{
